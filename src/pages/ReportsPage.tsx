...
import { conflicts, contractors, inspections, restorations } from '../data/mockData';
...
function ReportsPage() {
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="REPORTS AND ANALYTICS" title="Infrastructure Performance Reports" description="Review project, conflict, contractor, complaint, budget, inspection and restoration indicators." action={<button className="road-button road-button-primary" onClick={() => window.print()}>⇩ Export Report</button>} />
      ...
      <section className="stats-grid stats-grid-six">
        ...
        <StatCard label="Restoration Approved" value={restorations.filter((item) => item.status === 'Restoration Approved').length} note="Final quality gate" icon="▰" tone="green" delay={350} />
      </section>
      ...
      <section className="report-grid">
        ...
        <article className="content-card chart-card page-enter">
          <header className="content-card-header"><div><h2>Quality verification</h2><p>Inspection and restoration outcomes.</p></div></header>
          <div className="quality-donut-wrap">
            ...
            <div className="quality-legend">
              ...
              <span><i className="quality-warning" /> Pending restoration <strong>{restorations.filter((item) => item.status !== 'Restoration Approved').length}</strong></span>
            </div>
          </div>
        </article>
      </section>
